...
const CreateASchool:React.FC<Props> = () => {
    ...
    const save = (payload:any, setLoading:React.Dispatch<React.SetStateAction<boolean>>) => {
        dispatch(createSchoolEffect(
            {
                failCb: () => {
                    toast.error("Unable to create your school")
                },
                successCb: () => {
                    toast.success("Your school has been successfully created. Plesae login to access your dashboard.")
    ...
    return (
        <DefaultLayout school>
            <SchoolCreateUpdate submitFunc={save} />
        </DefaultLayout>
    )
}
